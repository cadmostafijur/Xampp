...
$sql_schedules = "SELECT schedule_id_2, departure_location, arrival_location FROM bus_schedules_2";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $departure_location = "BRAC University"; // default value
    ...
    if (!empty($selected_schedule)) {
        $schedule_id_2 = reset($selected_schedule)['schedule_id_2'];

        // Insert booking into database
        $sql_booking = "INSERT INTO bookings (user_id, schedule_id_2, departure_location, arrival_location, journey_date, price) VALUES ('$user_id', '$schedule_id_2', '$departure_location', '$arrival_location', '$journey_date', '$price')";
...
<body>
    <div class="container">
        <h2>Book Ticket</h2>
        <form action="book_ticket2.php" method="post">
            ...
            <input type="text" id="departure_location" name="departure_location" value="<?php echo ("BRAC University"); ?>" readonly><br>
            
            <label for="arrival_location">Arrival Location:</label><br>
            <select id="arrival_location" name="arrival_location" required>
                <?php foreach ($schedules as $schedule): ?>
                    <option value="<?php echo $schedule['arrival_location']; ?>"><?php echo $schedule['arrival_location']; ?></option>
                <?php endforeach; ?>
            </select><br>
